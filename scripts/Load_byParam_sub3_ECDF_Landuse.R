...
pdf(paste(outputDirectory, "Load_ECDFs.pdf", sep="/"), width=10, height=7.25)

for (i in 1:length(ParamList)) {
  
  ParamData  <- storm_load[which(storm_load$Parameter_string == ParamList[i]), ]
  ParamData<-ParamData[!is.na(ParamData$sample_loads),]
  ylimits <-  c(min(ParamData$sample_loads)/2, max(ParamData$sample_loads)*2)
  
  K_M <- cenfit(obs=ParamData$sample_loads, censored=ParamData$nonDetect_Flag, groups=ParamData$Type)
  KM_data <- summary(K_M)
